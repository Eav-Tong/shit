...
const DEEPSEEK_API_KEY = process.env.DEEPSEEK_API_KEY || 'sk-a5c9c3f9b49f49d8bd4b9ae0ebc3260e';
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/api/ai' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                const { type, messages } = JSON.parse(body);
...
绝望|无论你现在经历什么，请相信黑暗中一定会有光明。请联系身边信任的人，或拨打心理援助热线。`;
    ...
    let filePath = req.url === '/' ? '/study-pet.html' : req.url;
...
server.listen(PORT, () => {
    ...
    console.log(`使用 .env 中的 API Key: ${DEEPSEEK_API_KEY.substring(0, 10)}...`);
});
